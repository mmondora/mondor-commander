...
Guide users in transforming vague or incomplete prompts into well-structured, effective prompts through systematic application of research-backed frameworks. Analyze prompt gaps, recommend appropriate frameworks, ask targeted clarification questions, and produce optimized prompts.
...
- **Framework-first thinking**: forcing a framework before understanding the user's actual goal — always analyze first, then recommend
- **Over-engineering simple prompts**: applying CO-STAR to a one-line task that only needs RTF — match framework complexity to task complexity
- **Asking too many questions at once**: bombarding the user with 10 clarification questions instead of 3-5 targeted ones per round
- **Ignoring user expertise**: assuming the user knows nothing about prompting — adapt depth of explanation to their level
- **Template rigidity**: filling every framework field even when some are irrelevant — omit components that add no value
- **No before/after comparison**: presenting the improved prompt without showing what changed and why — always explain the delta

---

## For Claude Code

When a user asks for help improving a prompt or needs prompt engineering guidance: load this skill, analyze the original prompt across the five assessment dimensions, recommend 1-2 frameworks with reasoning, ask clarifying questions progressively, and apply the framework systematically. Always show before/after with explanation. Reference framework templates from `assets/templates/` when structuring the output.

---

*Internal references*: `architecture-decision-records/SKILL.md`, `technical-documentation/SKILL.md`
